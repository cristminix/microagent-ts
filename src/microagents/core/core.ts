      ...
      for (const match of matches) {
        ...
        if (innerContent) {
          try {
              ...
              root.each(
                (child, index, level) => {
                  if (child.node.nodeType === 1) {
                    // Node.ELEMENT_NODE
                    const toolName =
                      (child.node as any).localName ||
                      (child.node as any).nodeName
                    ...
                    if (toolName in this.tools) {
                      ...
                      child.each(
                        (paramElement, paramIndex, paramLevel) => {
                          if (paramElement.node.nodeType === 1) {
                            // Node.ELEMENT_NODE
                            const paramName =
                              (paramElement.node as any).localName ||
                              (paramElement.node as any).nodeName
                            const paramValue =
                              paramElement.node.textContent || "" // Get the text content of the element
                            ...
                            if (paramName in tool.parameters) {
                              ...
                              try {
                                ...
                                throw new Error(
                                  `Failed to convert parameter '${paramName}' value '${paramValue}' to type ${paramType}: ${
                                    (e as Error).message
                                  }`
                                )
                              }
                        ...
                        false,
                        false
                      ) // self=false, recursive=false for immediate children only
            ...
            throw new Error(
              `Failed to parse tool calls XML: ${(error as Error).message}`
            )
          }
        }
      }
