...
// Fungsi untuk menjalankan sampling prompt untuk tantangan pemrograman
async function runProgrammingChallengeSampling(inputPrompt: string, numSamplings: number, targetWords: number): Promise<any> {
    // Agent untuk prompt sampling tantangan pemrograman
    const samplingAgent = new MicroAgent(
        chatLLM,
        `Anda adalah agen sampling prompt untuk tantangan pemrograman. Tugas Anda adalah menghasilkan beberapa tantangan pemrograman dengan gaya platform pemrograman kompetitif (misalnya, LeetCode, AtCoder, Codeforces) sesuai dengan instruksi berikut:

Generate ${numSamplings} instances data based on the input prompt. Each instance should be about ${targetWords} words.

Tantangan pemrograman yang dihasilkan harus:
...
// Contoh penggunaan
async function main() {
    console.log("Contoh Prompt Sampling Tantangan Pemrograman dengan MicroAgent");
    console.log("=============================================================");

    // Contoh prompt sampling untuk tantangan pemrograman
    const inputPrompt = "Hasilkan tantangan pemrograman dengan gaya platform pemrograman kompetitif (misalnya, LeetCode, AtCoder, Codeforces).";
    ...
    try {
        const result = await runProgrammingChallengeSampling(inputPrompt, numSamplings, targetWords);
...
export { runProgrammingChallengeSampling };